...
var OmniClientCommands = {
    ...
    omniListUnspent: 'listunspent',
    omniCreatepayloadSimplesend: 'omni_createpayload_simplesend',
    omnicreaterawtxOpreturn: 'omni_createrawtx_opreturn',
    omnicreaterawtxReference: 'omni_createrawtx_reference',
    omnicreaterawtxChange: 'omni_createrawtx_change'
}
...
// 功能拓展 2018年03月21日14:16:42 by cf
Omni.listunspent = function(min, max, address, cb) {
    var min = min || 1
    var max = max || 999999
    client.cmd('listunspent', min, max, address, function(err, data, resHeaders) {
...
Omni.createpayload_simplesend = function(identifier, amount, cb) {
    var identifier = identifier || 31
    var amount = amount || '0.00001'
    client.cmd('omni_createpayload_simplesend', identifier, amount, function(err, data, resHeaders) {
        if (err) { return cb(err) } else {
            return cb(data)
        }
    })
...
Omni.createrawtx_opreturn = function(base, payload, cb) {
    client.cmd('omni_createrawtx_opreturn', base, payload, function(err, data, resHeaders) {
        if (err) { return cb(err) } else {
            return cb(data)
        }
    })
...
Omni.createrawtx_reference = function(code, receiver, cb) {
    client.cmd('omni_createrawtx_reference', code, receiver, function(err, data, resHeaders) {
        if (err) { return cb(err) } else {
            return cb(data)
        }
    })
...
Omni.createrawtx_change = function(reference, ins, receiver, fee, cb) {
    // ins: txid, vout, scriptPubKey and value
    var position = 1
    client.cmd('omni_createrawtx_change', reference, ins, receiver, fee, position, function(err, data, resHeaders) {
        if (err) { return cb(err) } else {
            return cb(data)
        }
    })
...
Omni.createrawtx_multisig = function(create, payload, cpn_address, cpn_privatekey, cb) {
    client.cmd('omni_createrawtx_multisig', create, payload, cpn_address, cpn_privatekey, function(err, data, resHeaders) {
        if (err) { return cb(err) } else {
            return cb(data)
        }
    })
...
// Omni.signrawtransaction = function(code, cb) {
//         client.cmd('signrawtransaction', code, function(err, data, resHeaders) {
//             if (err) { return cb(err) } else {
//                 return cb(data)
//             }
//         })
//     }
Omni.signrawtransaction = function(code, tx, privatekey, sighash, cb) {
    sighash = sighash || 'ALL'
    client.cmd('signrawtransaction', code, tx, privatekey, sighash, function(err, data, resHeaders) {
        if (err) { return cb(err) } else {
            return cb(data)
        }
    })
...
Omni.signrawtransaction_withdraw = function(code, cb) {
    client.cmd('signrawtransaction', code, function(err, data, resHeaders) {
        if (err) { return cb(err) } else {
            return cb(data)
        }
...
Omni.sendrawtransaction = function(code, cb) {
    client.cmd('sendrawtransaction', code, function(err, data, resHeaders) {
        if (err) { return cb(err) } else {
            return cb(data)
        }
    })
...
Omni.init = function(user, pass, otherip, otherport) {
    var host
    if (otherip == null) { host = 'localhost' } else { host = otherip }
    var port
    if (typeof otherport == 'boolean') {
...
Omni.importaddress = function(address, account, rescan, cb) {
    client.cmd('importaddress', address, account, rescan, function(err, data, resHeaders) {
        if (err) {
            return cb(err)
        } else {
            return cb(data)
...
Omni.listtransactions = function(txid, count, skip, startblock, endblock, cb) {
    ...
    // client.cmd("listtransactions", txid, count, skip, startblock, endblock, function(err, data, resHeaders) {
